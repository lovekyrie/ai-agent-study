import type { VectorDocument, SearchResult, SearchOptions, CollectionStats, ICollection } from './types.js'
...
export class InMemoryCollection implements ICollection {
  ...
  async update(
    ...
    if (update.metadata !== undefined) doc.metadata = update.metadata
    if (update.embedding !== undefined) doc.embedding = update.embedding
  }
