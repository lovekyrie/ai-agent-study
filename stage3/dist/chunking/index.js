 ...
 * 按行切片（适合代码）
 */
export function chunkByLines(content, linesPerChunk = 30, overlapLines = 5) {
 ...
 * 按字符切片（通用）
 */
export function chunkByCharacters(content, maxSize, overlap) {
 ...
 * 智能切片（代码优先按行，其他按字符）
 */
export function chunkContent(content, filePath, config = {}) {
 ...
 * 扫描目录下所有文件
 */
export async function scanDirectory(dirPath, filter = {}) {
    const { includeExtensions, excludeDirs = ['node_modules', '.git', 'dist', 'build', '.next', '__pycache__'], maxFileSize = 500 * 1024, // 默认 500KB
     } = filter;
    const patterns = [];
 ...
 * 扫描目录并切片所有文件
 */
export async function processDirectory(dirPath, filter = {}, config = {}) {
